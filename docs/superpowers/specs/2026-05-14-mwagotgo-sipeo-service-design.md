...
로그인 페이지와 홈의 계정 CTA는 같은 분기 규칙을 사용합니다.

- 세션이 없으면 로그인 CTA를 보여주고 `/login`으로 이동합니다.
- 로그인했지만 온보딩을 완료하지 않은 사용자는 `위시리스트 만들기 계속하기` CTA를 보여주고 `/onboarding`으로 이동합니다.
- 로그인했고 온보딩을 완료한 사용자는 `내 위시리스트 관리하기` CTA를 보여주고 `/admin`으로 이동합니다.
